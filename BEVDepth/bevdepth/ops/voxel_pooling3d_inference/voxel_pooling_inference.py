...
from . import voxel_pooling3d_inference_ext


class VoxelPooling3DInference(Function):
    ...
    def forward(ctx, geom_xyz: torch.Tensor, depth_features: torch.Tensor,
        ...
        output_features = depth_features.new_zeros(
            (batch_size, voxel_num[2], voxel_num[1], voxel_num[0], num_channels))
        voxel_pooling3d_inference_ext.voxel_pooling3d_inference_forward_wrapper(
            batch_size,
            num_cams,
            num_depth,
            num_height,
...
voxel_pooling3d_inference = VoxelPooling3DInference.apply
