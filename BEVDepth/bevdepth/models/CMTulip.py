...
class CMTULIP(nn.Module):
    def __init__(self, backbone_config,
                 img_size = (32, 2048), target_img_size = (128, 2048) ,patch_size = (4, 4), in_chans: int = 1, embed_dim: int = 96,
        ...
        self.backbone = FusionLSSFPN(**backbone_config)
    ...
    def forward(self, x, in_imgs, mats_dict, timestamps, target, eval = False, mc_drop = False):
        ...
        if self.is_train_depth and self.training:
            img_feat, depth_pred = self.backbone(in_imgs,
                                                    mats_dict,
                                                    timestamps,
                                                    is_return_depth=True)
        else:
            img_feat = self.backbone(in_imgs, mats_dict, timestamps)
