#!/bin/bash -l 
#SBATCH --export=ALL
#SBATCH --job-name=TULIP_kitti_train               # your job name
...
export DATA_DIR="${WORK}/data/kitti/"
...
apptainer exec --nv \
  --bind "${BIND_LIST}" \
  "${SIF_IMAGE}" \
  bash -lc 'source /opt/conda/etc/profile.d/conda.sh && conda activate py38 && cd "$CODE_DIR" && ./bash_scripts/tulip_upsampling_kitti.sh "${SLURM_GPUS_ON_NODE}"'