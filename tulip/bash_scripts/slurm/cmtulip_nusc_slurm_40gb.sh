#!/bin/bash -l 
#SBATCH --export=ALL
#SBATCH --job-name=CMTULIP_train_b8               # your job name
#SBATCH --nodes=1                          # 1 node
#SBATCH --ntasks-per-node=1                # one srun task per node
#SBATCH --gres=gpu:a100:8               # 8 GPUs on that node
...
apptainer exec --nv \
  --bind "${BIND_LIST}" \
  "${SIF_IMAGE}" \
  bash -lc 'source /opt/conda/etc/profile.d/conda.sh && conda activate py38 && cd "$CODE_DIR" && ./bash_scripts/cmtulip_upsampling_nusc_40gb.sh "${SLURM_GPUS_ON_NODE}"'