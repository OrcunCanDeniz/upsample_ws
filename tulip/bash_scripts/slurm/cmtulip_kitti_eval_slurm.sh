#!/bin/bash -l 
#SBATCH --export=ALL
#SBATCH --job-name=CMTULIP_eval_kitti     # your job name
...
apptainer exec --nv \
  --bind "${BIND_LIST}" \
  "${SIF_IMAGE}" \
  bash -lc 'source /opt/conda/etc/profile.d/conda.sh && conda activate py38 && cd "$CODE_DIR" && ./bash_scripts/cmtulip_upsampling_kitti.sh "${SLURM_GPUS_ON_NODE}" eval'