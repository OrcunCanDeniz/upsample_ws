...
def generate_info(nusc, scenes, split_name, data_root, max_cam_sweeps=1, max_lidar_sweeps=1, generate_rv=True):
    ...
    if generate_rv:
        ...
        print(f"RV generation disabled, using existing files in: {split_rv_dir}")
    ...
    for cur_scene in tqdm(nusc.scene):
        ...
        while True:
            ...
            # Get lidar pose for computing sensor2lidar transformations (needed for SCA dataset)
            ...
            for cam_name in cam_names:
                ...
                # Compute sensor2lidar transformation for SCA dataset compatibility
            ...
            # Convert to range image using the new converter (if enabled)
...
def main(generate_rv=False):
    ...
    train_infos = generate_info(trainval_nusc, train_scenes, 'train', data_root, generate_rv=generate_rv)
    val_infos = generate_info(trainval_nusc, val_scenes, 'val', data_root, generate_rv=generate_rv)
    ...
    test_infos = generate_info(test_nusc, test_scenes, 'test', data_root, generate_rv=generate_rv)
