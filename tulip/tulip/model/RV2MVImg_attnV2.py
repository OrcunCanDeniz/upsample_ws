...
from .tulip import WindowAttention, DropPath
import pdb
...
class CrossModalSCA(nn.Module):
    def __init__(self, embed_dims=128, msda_points=8, num_cams=6, dropout=0.1, num_levels=4, 
        ...
        _, self.norm_win_attn = build_norm_layer(dict(type='LN'), num_features=embed_dims)
    ...
    def forward(self, query, img_feats, reference_points_cam, mask):
        ...
        if query.dim() == 3:
            B, N, C_rv = query.shape
            query = query.view(B, self.in_h, self.in_w, C_rv)  # [B, Hrv, Wrv, C_rv]
        elif query.dim() == 4:
            query = query.permute(0,2,3,1).contiguous() # [B, Hrv, Wrv, C_rv]
        else:
            raise ValueError("Wrong input dimension for query in CrossModalSCA")

        query_copy = query
        query = self.norm1(query)

        query = self.window_attn(query)
        query = self.drop_path(query)
        query = query + query_copy

        query_copy = query
        query = self.norm_win_attn(query)

        query = query.flatten(1,2) # [B, N, C_rv], N must equal N used in point_sampling
...
class RV2MVImgAttn(nn.Module):
    ...
    def forward(self, in_rv_feat, img_feats, lidar2img_rts, img_shapes, return_nhwc=False, lr_depths=None):
        ...
        for i, layer in enumerate(self.cross_modal_sca_layers):
